...
wp plugin is-installed redis-cache --quiet --skip-plugins 2>/dev/null

if [ "$?" -eq 0 ]
then
   wp plugin is-active redis-cache --quiet --skip-plugins 2>/dev/null
...
# commercial version 
wp plugin is-installed object-cache-pro --quiet --skip-plugins 2>/dev/null
